  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      <ul className="space-y-4">
        {data.map(task => (
          <li key={task.id} className="flex items-start justify-between bg-white p-4 rounded-2xl shadow">
            <div className="flex items-start gap-3">
              <button className="mt-1 text-gray-500 hover:text-green-600">
                {task.completed ? <CheckCircle2 className="w-6 h-6 text-green-600" /> : <Circle className="w-6 h-6" />}
              </button>
              <div>
